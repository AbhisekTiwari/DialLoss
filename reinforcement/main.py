...
# vocab_size_trg = len(TRG.vocab)
vocab_size_trg = 30522
...
class BaselineEstimator(nn.Module):
    def __init__(self,device):
        ...
        self.device=device
        self.fc1 = nn.Linear(80*vocab_size_trg,128 )
        # self.fc2 = nn.Linear(1024, 128)
        self.fc3 = nn.Linear(128,1)
    
    def forward(self, x):
        x = x.view(-1).to(self.device) # flatten the input tensor
...
def calculate_scores(data, src_field, trg_field, model, device, max_len):
    ...
    scores=imported_calculate_scores(pred_sentences,trg_sentences)
...
if __name__ == "__main__":
    ...
    model_log_save_path = args.save + '.transformer.log'
    ...
    bse_model= BaselineEstimator(device).to(device)
    ...
    bse_optimizer= optim.Adam(bse_model.parameters(), lr=LR)
    ...
    module_path = '/mnt/Data/abhisek_1921cs16/ujjwalData/Dataset/reinforcement'
    ...
    if args.test_only.lower() == "no":
        for epoch in range(N_EPOCHS):
            ...
            train_loss, train_bse_loss = train(
                model,bse_model, train_iter, optimizer,bse_optimizer, criterion, CLIP, epoch + 1, N_EPOCHS,TRG.vocab.stoi[TRG.eos_token],
                SRC,TRG,embed_model
            )
            valid_loss, valid_bse_loss = evaluate(
                model,bse_model, valid_iter, criterion,TRG.vocab.stoi[TRG.eos_token],
                SRC,TRG,embed_model
            )

            # if valid_loss < best_valid_loss and epoch>11:
            #     best_valid_loss = valid_loss
            #     torch.save(model.state_dict(), model_save_path)

            # valid_bleu, _,valid_bert, valid_rouge = calculate_bleu(valid_set, SRC, TRG, model, device, MAX_LEN)
            # test_bleu, _,test_bert,test_rouge = calculate_bleu(test_set, SRC, TRG, model, device, MAX_LEN)
            ...
            logging.info(
                f"\tTrain bse Loss: {train_bse_loss:.3f} | Train PPL: {math.exp(train_bse_loss):7.3f}"
            )
            logging.info(
                f"\t Val. bse Loss: {valid_bse_loss:.3f} |  Val. PPL: {math.exp(valid_bse_loss):7.3f}"
            )
            ...
            f_log.write(f'\t Train BSE Loss: {train_bse_loss:.3f} | Train PPL: {math.exp(train_bse_loss):7.3f}')
            f_log.write(f'\t Val. BSE Loss: {valid_bse_loss:.3f} |  Val. PPL: {math.exp(valid_bse_loss):7.3f}')
            for key,val in score_dictionary_valid.items():
    ...
    test_loss,test_bse_loss = evaluate(model,bse_model,test_iter, criterion,TRG.vocab.stoi[TRG.eos_token],SRC,TRG,embed_model)
